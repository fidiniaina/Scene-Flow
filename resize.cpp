// taken from cvimgwarp.cpp and OpenCV includes
#include "resize.h"
...
CV_IMPL void
cvResizeScale( const CvArr* srcarr, CvArr* dstarr, int method, float scale_x_, float scale_y_ )
{
    static CvFuncTable bilin_tab, bicube_tab, areafast_tab, area_tab;
    static int inittab = 0;
    void* temp_buf = 0;

    CV_FUNCNAME( "cvResizeScale" );
    ...
    if (scale_x_ == 0)
        ...
        assert(dsize.width >= cvFloor(scale_x_*ssize.width) &&
               dsize.width <= cvCeil(scale_x_*ssize.width));
        scale_x = 1./scale_x_;
    }
    if (scale_y_ == 0)
        scale_y = (float)ssize.height/dsize.height;
    else {
        assert(dsize.height >= cvFloor(scale_y_*ssize.height) &&
               dsize.height <= cvCeil(scale_y_*ssize.height));
        scale_y = 1./scale_y_;
    }
